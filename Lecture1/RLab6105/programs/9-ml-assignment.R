...
library(e1071)
set.seed(7)
fit.lda <- train(Treatment~., data=dataset, method="lda", metric=metric, trControl=control)


# b) nonlinear algorithms
# kNN
set.seed(7)
fit.knn <- train(Treatment~., data=dataset, method="knn", metric=metric, trControl=control)

# c) SVMLinear2 algorithm 
set.seed(7)
fit.svnLinear2 <-train(Treatment~., data=dataset, method="svmLinear2", metric=metric, trControl=control)

# d) Random Forest Algorithm
set.seed(7)
fit.rf <-train(Treatment~., data=dataset, method="rf", metric=metric, trControl=control)

# e) Ada Boost Classification Algortihm
set.seed(7)
fit.adaboost <- train(Treatment~., data=dataset, method="adaboost", metric=metric, trControl=control)
...
confusionMatrix(predictions, validation$Treatment)
